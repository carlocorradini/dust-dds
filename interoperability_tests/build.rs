...
fn main() {
    ...
    let idl_path = Path::new("Inheritance.idl");
    let compiled_idl = dust_dds_gen::compile_idl(idl_path).expect("Couldn't parse IDL file");
    let compiled_idl_path = build_path.join("inheritance.rs");
    let mut file = File::create(compiled_idl_path).expect("Failed to create file");
    file.write_all(compiled_idl.as_bytes())
        .expect("Failed to write to file");

    let idl_path = Path::new("Union.idl");
    let compiled_idl = dust_dds_gen::compile_idl(idl_path).expect("Couldn't parse IDL file");
    let compiled_idl_path = build_path.join("union.rs");
    let mut file = File::create(compiled_idl_path).expect("Failed to create file");
    file.write_all(compiled_idl.as_bytes())
        .expect("Failed to write to file");

    let idl_path = Path::new("Dummy.idl");
    let compiled_idl = dust_dds_gen::compile_idl(idl_path).expect("Couldn't parse IDL file");
    let compiled_idl_path = build_path.join("dummy.rs");
    let mut file = File::create(compiled_idl_path).expect("Failed to create file");
    file.write_all(compiled_idl.as_bytes())
        .expect("Failed to write to file");
}
