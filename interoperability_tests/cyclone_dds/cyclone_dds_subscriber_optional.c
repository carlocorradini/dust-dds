...
int main(int argc, char *argv[])
{
    ...
    const dds_entity_t topic = dds_create_topic(participant, &interoperability_test_OptionalType_desc, topic_name, NULL /*qos*/, NULL /*listener*/);
    if (topic < 0)
    {
    ...
    interoperability_test_OptionalType *msg;
    void *samples[MAX_SAMPLES];
    dds_sample_info_t infos[MAX_SAMPLES];
    samples[0] = interoperability_test_OptionalType__alloc();
    ...
    {
        msg = (interoperability_test_OptionalType *)samples[0];
        printf("Received: %s { maybe_a: %p, maybe_b: %p, maybe_c: %p }\n", interoperability_test_OptionalType_desc.m_typename, (void *)msg->maybe_a, (void *)msg->maybe_b, (void *)msg->maybe_c);
    }
