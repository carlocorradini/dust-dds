...
int main(int argc, char *argv[]) {
  ...
  const dds_entity_t data_writer =
      dds_create_writer(participant, topic, qos, NULL /*listener*/);
  if (data_writer < 0) {
  ...
  uint8_t value = 0;
  while (true) {
    interoperability_test_Dummy data = {
        .id = "Dummy",
        .value = value,
    };
    printf("write \"%s\": { id: \"%s\", value: %d }\n",
           interoperability_test_Dummy_desc.m_typename, data.id, data.value);
    rc = dds_write(data_writer, &data);
    if (rc != DDS_RETCODE_OK) {
